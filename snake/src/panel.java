import javax.swing.*;   //use for graphics
import java.awt.*;
import java.awt.event.*;  //use for movement of snake
import java.util.Arrays;
import java.util.Random;

public class panel extends JPanel implements ActionListener {  //J/panel:provide space in apps to add componen
     static int width=1200;
     static int height=600;
     static int unit=50;
     Timer timer;  //to specify time in which new frame display
    ...
    boolean flag=false; //specify game has ended
    ...
    panel(){
        ...
        this.setFocusable(true); //allow keyboard input to process
        random=new Random(); //
    ...
    public void draw(Graphics graphic) {
        if (flag == true) {
            ...
            graphic.setFont(new Font("Comics sans", Font.BOLD, 40));
...
//display game over text
    graphic.setColor(Color.red);
    graphic.setFont(new Font("Comics sans", Font.BOLD, 80));
...
public void move(){
        //updating all body part except head
        for(int i=length;i > 0; i--){
            xsnake[i]=xsnake[i-1];
            ysnake[i]=ysnake[i-1];
        }
